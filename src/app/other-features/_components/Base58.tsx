import { Base58Schema, bash58Schema } from "@/utils/schema";
...
import { getBase58Address } from "@/actions/getBase58Address";

function Base58() {
  const [address, setAddress] = useState<string>("");

  async function onSubmit(values: Base58Schema) {
    const base58address = await getBase58Address({
      prefix: parseInt(values.prefix, 16),
      hash: values.hash,
    });

    setAddress(base58address.address);
  }

  const form: UseFormReturn<Base58Schema> = useForm<Base58Schema>({
    mode: "all",
    resolver: zodResolver(bash58Schema),
    defaultValues: {
      prefix: "00",
      hash: "",
    },
